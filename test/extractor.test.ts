...
test("content detection helpers", () => {
	...
	assert.equal(parseGitHubUrl("https://github.com/acme/project/blob/main/src/a.ts")?.path, "src/a.ts");
	assert.equal(parseYouTubeVideoId("https://youtu.be/dQw4w9WgXcQ"), "dQw4w9WgXcQ");
	assert.equal(calculateQuality({ contentLength: 0, wordCount: 1000, hasTitle: true, hasMetadata: true }), 80);
});
...
test("URL validation rejects unsafe schemes and private destinations", async () => {
